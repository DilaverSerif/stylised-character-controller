...
public class PhysicsBasedCharacterController : MonoBehaviour
{
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        Vector3 rayStart = _cachedTransform.position;
        Vector3 rayEnd = rayStart + (Vector3Down * _rayToGroundLength);
        Gizmos.DrawLine(rayStart, rayEnd);

        // Draw ride height sphere
        Gizmos.color = Color.green;
        Vector3 rideHeightPos = rayStart + (Vector3Down * _rideHeight);
        Gizmos.DrawWireSphere(rideHeightPos, 0.1f);

        // Draw grounded tolerance sphere
        Gizmos.color = Color.cyan;
        Vector3 tolerancePos = rayStart + (Vector3Down * (_rideHeight * GROUNDED_DISTANCE_TOLERANCE));
        Gizmos.DrawWireSphere(tolerancePos, 0.1f);
        
        // Draw ground snap distance if enabled
        if (_groundSnapDistance > EPSILON_DISTANCE)
        {
            Gizmos.color = Color.magenta;
            Vector3 snapPos = rayStart + (Vector3Down * (_rideHeight + _groundSnapDistance));
            Gizmos.DrawWireSphere(snapPos, 0.08f);
        }
        
        // Draw ground normal if grounded
        if (_isGrounded && _groundAngle > EPSILON_DISTANCE)
        {
            Gizmos.color = Color.red;
            Gizmos.DrawRay(rayStart, _groundNormal * 0.5f);
        }
